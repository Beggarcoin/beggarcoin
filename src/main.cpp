...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Beggarcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Beggarcoin Signed Message:\n";
...
{
    // Beggarcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 20 * 60; // Beggarcoin: 20 minutes
static const int64 nTargetSpacing = 60; // Beggarcoin: 1 minute
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Beggarcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Beggarcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Beggarcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Beggarcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BeggarcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BeggarcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BeggarcoinMiner(CWallet *pwallet)
{
    printf("BeggarcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("beggarcoin-miner");
    ...
    try { loop {
        ...
        printf("Running BeggarcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BeggarcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BeggarcoinMiner, pwallet));
}
